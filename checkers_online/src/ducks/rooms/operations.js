...
export const getOneRoomOperation = (roomId) => {
  return createAction({
    endpoint: `http://localhost:5000/rooms/getOneRoom/${roomId}`,
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
    types: [
      "types.ROOMS_GET_ONE_ROOM_FROM_DATA_BASE_START",
      types.ROOMS_GET_ONE_ROOM_FROM_DATA_BASE_SUCCESS,
      {
        type: types.ROOMS_GET_ONE_ROOM_FROM_DATA_BASE_FAILURE,
        payload: async (action, state, res) => {
          const json = await res.json();
          return json;
        },
